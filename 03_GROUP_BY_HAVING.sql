...
SELECT EMPLOYEE.DEPT_CODE 부서코드, 
ROUND(AVG(SALARY)) 평균급여, COUNT(*) "인원 수"
FROM EMPLOYEE
...
FROM EMPLOYEE
WHERE EMPLOYEE.HIRE_DATE >'2000-01-01'
GROUP BY EMPLOYEE.JOB_CODE 
ORDER BY JOB_CODE;

--------------------------------------------------

-- * 여러 칼럼을 묶어서 그룹으로 지정 가능 --> 그룹 내 그룹이 가능하다! *
...
SELECT EMPLOYEE.DEPT_CODE 부서코드, EMPLOYEE.JOB_CODE 직급코드,
COUNT(*)
FROM EMPLOYEE
GROUP BY EMPLOYEE.DEPT_CODE, EMPLOYEE.JOB_CODE 
ORDER BY 부서코드 , 직급코드 DESC;
...
SELECT EMPLOYEE.DEPT_CODE 부서코드, ROUND(AVG(SALARY)) 평균급여
FROM EMPLOYEE
GROUP BY DEPT_CODE
HAVING AVG(EMPLOYEE.SALARY) >= 3000000
ORDER BY DEPT_CODE;
...
SELECT EMPLOYEE.JOB_CODE 직급코드, COUNT(*)
FROM EMPLOYEE
GROUP BY EMPLOYEE.JOB_CODE 
HAVING COUNT(*)<6 -- HAVING 절에는 그룹함수가 반드시 작성된다!
ORDER BY 직급코드;
...
FROM EMPLOYEE
GROUP BY ROLLUP(EMPLOYEE.DEPT_CODE , JOB_CODE)
ORDER BY 1;
...
SELECT DEPT_CODE, JOB_CODE, COUNT(*)
FROM EMPLOYEE
GROUP BY CUBE(EMPLOYEE.DEPT_CODE , JOB_CODE)
ORDER BY 1;
