...
-- SELECT EMP_NAME + 10 FROM EMPLOYEE; <- 오류!!
...
-- DUAL : ORACLE에서 사용하는 더미테이블.
-- 더미테이블? 실제 데이터를 저장하는게 아닌, 임시 계산이나 테스트 목적으로 사용.
...
-- 4. ORDER BY 절(정렬) : ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC] | [NULLS FIRST | LAST]
...
-- 사번, 이름, 급여, 전화번호 조회
SELECT EMPLOYEE.EMP_ID ,EMPLOYEE.EMP_NAME ,EMPLOYEE.SALARY , EMPLOYEE.PHONE 
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서
-- 급여가 300만원 이상 그리고 600만원 이하인 사원의
-- 사번, 이름, 급여, 전화번호 조회
SELECT EMP_ID, EMP_NAME, SALARY, PHONE
FROM EMPLOYEE
...
-- 이름, 입사일 조회
SELECT EMPLOYEE.EMP_NAME , EMPLOYEE.HIRE_DATE 
FROM EMPLOYEE
...
-- EMLOYEE 테이블에서 전화번호가 010으로 시작하지 않는 사원
-- 사번, 이름, 전화번호 조회
...
-- EMPLOYEE 테이블에서
-- 이메일 '_'앞이 4글자 이면서
-- 부서코드가 'D9' 또는 'D6'이고 --> AND가 OR보다 우선순위 높다, 소괄호() 사용가능
...
FROM EMPLOYEE
ORDER BY SALARY; -- ASC(오름차순)가 기본값 [ASC | DESC(내림차순)]
-- 컬럼명 사용.
