...
-- ANSI
-- 연결에 사용하 컬럼명이 같은 경우 USING(컬럼명) 을 사용할 수 있다.
...
-- 1) LEFT [OUTER] JOIN : 합치기에 사용한 두 테이블 중 외편에
-- 기술된 테이블의 컬럼수를 기준으로 JOIN
...
ON(DEPT_CODE = d.DEPT_ID ); -- 23행(DEPT_CDOE가 NULL인 하동운, 이오리 포함)

-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE e , DEPARTMENT
WHERE DEPT_CODE = DEPARTMENT.DEPT_ID(+) ;
...
--> JOIN 구문 잘못 작성하는경우 CROSS JOIN의 결과가 조회됨

SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE CROSS JOIN DEPARTMENT d ;
...
WHERE DEPT_CODE = d.DEPT_ID -- (EMPLOYEE + DEPARTMENT 조인)
AND LOCATION_ID = LOCAL_CODE; -- (DEPARTMENT + LOCATION 조인)
...
SELECT e.EMP_ID 사번, e.EMP_NAME 이름, j.JOB_NAME 직급명, DEPARTMENT.DEPT_TITLE 부서명, LOCATION.LOCAL_NAME 근무지역명, e.SALARY 급여
FROM EMPLOYEE e 
JOIN JOB j ON (e.JOB_CODE = j.JOB_CODE ) 
JOIN DEPARTMENT ON(DEPT_CODE= DEPT_ID) 
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE J.JOB_CODE = 'J6' AND LOCATION.LOCAL_CODE IN('L1','L2','L3');
...
FROM EMPLOYEE
LEFT JOIN DEPARTMENT d ON(EMPLOYEE.DEPT_CODE  = d.DEPT_ID )
...
SELECT EMP_NAME, JOB_NAME, SALARY, CASE 
WHEN e.BONUS IS NOT NULL THEN SALARY*(BONUS+1)*12
ELSE SALARY*12 END 연봉
FROM EMPLOYEE e 
JOIN JOB j USING (JOB_CODE)
