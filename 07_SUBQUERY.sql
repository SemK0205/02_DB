...
WHERE DEPT_CODE =(
	SELECT e.DEPT_CODE 
	FROM EMPLOYEE e 
	WHERE EMP_NAME = '노옹철') AND e.EMP_NAME != '노옹철';
...
-- 서브쿼리
SELECT MIN(SALARY) FROM EMPLOYEE e ; -- 1,380,000
...
WHERE SALARY > 
(SELECT SALARY FROM EMPLOYEE e WHERE EMP_NAME = '노옹철');
...
SELECT MAX(SALARY)
FROM EMPLOYEE e 
GROUP BY e.DEPT_CODE ; -- 7행 1열 (다중행)
...
-- * 사수 == MANAGER_ID 컬럼에 작성된 사번을 가지고 있는 사람
SELECT * FROM EMPLOYEE;

-- 1) 사수에 해당하는 직원에 대해 조회 (서브쿼리)
...
ON(d.DEPT_ID = EMPLOYEE.DEPT_CODE );

-- 3) 사수에 해당하는 직원에 대한 정보 추출 조회 (구분 '사수')
SELECT EMP_ID, EMP_NAME, DEPT_TITLE, JOB_NAME, '사수' 구분 
FROM EMPLOYEE
JOIN JOB j USING(JOB_CODE)
LEFT JOIN DEPARTMENT d ON(d.DEPT_ID = EMPLOYEE.DEPT_CODE )
WHERE EMP_ID IN (
SELECT DISTINCT MANAGER_ID
FROM EMPLOYEE e 
WHERE e.MANAGER_ID IS NOT NULL);
...
-- 3) EMPLOYEE 테이블의 DEPT_CODE와 동일한 사원 조회
...
CASE WHEN DEPT_TITLE IS NULL THEN '소속없음'
ELSE DEPT_TITLE END, JOB_NAME, HIRE_DATE 
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
LEFT JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE)
WHERE 
HIRE_DATE IN (SELECT MIN(HIRE_DATE)
FROM EMPLOYEE
WHERE ENT_YN = 'N'
GROUP BY DEPT_CODE)
ORDER BY 6;
...
-- SQL에서 단일값 '스칼라' 라고함.
-- 즉, SELECT 절에 작성되는 단일열 서브쿼리를 스칼라 서브쿼리라고 함.
...
-- 메인쿼리
SELECT EMP_NAME, JOB_CODE, SALARY, 
(SELECT AVG(SALARY) FROM EMPLOYEE GROUP BY JOB_CODE) "급여 평균"
FROM EMPLOYEE;
...
(SELECT CEIL(AVG(SALARY))
FROM EMPLOYEE SUB
WHERE SUB.JOB_CODE = MAIN.JOB_CODE) "급여 평균"
FROM EMPLOYEE MAIN
ORDER BY JOB_CODE ;
...
ELSE (SELECT EMP_NAME FROM EMPLOYEE SUB
WHERE SUB.EMP_ID  = MAIN.MANAGER_ID) END 관리자명
FROM EMPLOYEE MAIN;
...
SELECT ROWNUM 순위, EMP_NAME 이름, SALARY
...
-- 부서코드, 부서명, 평균급여 조회
-- 인라인뷰 (+ GROUP BY)
SELECT DEPT_CODE 부서코드,DEPT_TITLE 부서명, 평균급여
...
-- EX) 공동 1위가 2명 다음순위 2 순위가 아닌 3위로
...
-- DENSE_RANK() OVER : 건너뛰지 않고 순위 계산
-- EX) 공동 1위가 2명 다음순위 2위
