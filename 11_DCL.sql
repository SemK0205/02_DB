...
CREATE USER ksw_sample IDENTIFIED BY 1234;

-- 2. 새 연결(접속) 추가
-- ORA-01045: 사용자 KSW_SAMPLE는 CREATE SESSION 권한을 가지고있지 않음; 로그온이 거절되었습니다
--> 접속 권한

-- 3. 접속 권한 부여 (sys -> 사용자 계정)
-- [권한 부여 작성법]
-- GRANT 권한, 권한, 권한... TO 사용자명;
GRANT CREATE SESSION TO ksw_sample;
...
GRANT CREATE TABLE TO ksw_sample;

ALTER USER ksw_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
...
GRANT CONNECT, RESOURCE TO ksw_sample;
...
GRANT SELECT ON EMPLOYEE TO ksw_sample;


-- 3. (sample) 다시 kh 계정의 EMPLOYEE 조회
SELECT * FROM kh.EMPLOYEE; --> 조회 잘 된다!
SELECT * FROM kh.DEPARTMENT; --> 조회 안된다 / ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
...
REVOKE SELECT ON EMPLOYEE FROM ksw_sample;

-- 5. (sample) EMPLOYEE 조회해보기
SELECT * FROM kh.EMPLOYEE e ; --> 조회 안된다.
