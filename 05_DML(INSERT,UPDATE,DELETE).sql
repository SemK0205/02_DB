...
'01011111111', 'D1', 'J7', 'S3', 4300000, 0.2, 200, 
SYSDATE, NULL, 'N');
...
-- 선택 안된 컬럼은 값이 NULL이 들어감 (DEFAULT 존재 시 DEFAULT 설정
-- 한 값이 삽입됨)
...
CREATE TABLE EMP_01(EMP_ID NUMBER, EMP_NAME VARCHAR2(30),
...
FROM EMPLOYEE2 e 
LEFT JOIN DEPARTMENT2 d ON(DEPT_CODE = d.DEPT_ID );
...
-- 유재식 급여 조회
SELECT EMPLOYEE2.SALARY FROM EMPLOYEE2
WHERE EMPLOYEE2.EMP_NAME = '유재식'; -- 3,400,000

-- 유재식 보너스율 조회
SELECT BONUS FROM EMPLOYEE2
WHERE EMPLOYEE2.EMP_NAME = '유재식'; -- 0.2


-- 방명수 급여, 보너스 수정
UPDATE EMPLOYEE2 e 
SET SALARY = (SELECT EMPLOYEE2.SALARY FROM EMPLOYEE2
WHERE EMPLOYEE2.EMP_NAME = '유재식'),
BONUS = (SELECT BONUS FROM EMPLOYEE2
WHERE EMPLOYEE2.EMP_NAME = '유재식')
WHERE e.EMP_NAME = '방명수';

SELECT EMP_NAME, SALARY, BONUS FROM EMPLOYEE2 e 
WHERE e.EMP_NAME IN('방명수','유재식');
